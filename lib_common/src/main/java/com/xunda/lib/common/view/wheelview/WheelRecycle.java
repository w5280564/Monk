...
public class WheelRecycle {
	 ...
	 * @param firstItem the number of first item_choose_image in layout
	 * @param range the range of current wheel items 
	 * @return the new values of first item_choose_image number
	 */
	public int recycleItems(LinearLayout layout, int firstItem, ItemsRange range) {
		...
		for (int i = 0; i < layout.getChildCount();) {
			if (!range.contains(index)) {
				...
				if (i == 0) { // first item_choose_image
					firstItem++;
				}
			} else {
				i++; // go to next item_choose_image
			}
			index++;
		}
		return firstItem;
	...
	/**
	 * Gets empty item_choose_image view
	...
	/**
	 * Adds view to cache. Determines view type (item_choose_image view or empty one) by index.
	 * @param view the view to be cached
