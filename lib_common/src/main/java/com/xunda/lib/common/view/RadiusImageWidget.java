...
public class RadiusImageWidget extends ImageView {
    ...
    public RadiusImageWidget(Context context) {
        super(context);
    }

    public RadiusImageWidget(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RadiusImageWidget(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.RadiosImageView, defStyle, 0);

        final int index = a.getInt(R.styleable.RadiosImageView_android_scaleType, -1);
        if (index >= 0) {
            setScaleType(sScaleTypeArray[index]);
        }

        mLeftTopCornerRadius = a.getDimensionPixelSize(
                R.styleable.RadiosImageView_left_top_radius, 0);
        mRightTopCornerRadius = a.getDimensionPixelSize(
                R.styleable.RadiosImageView_right_top_radius, 0);
        mLeftBottomCornerRadius = a.getDimensionPixelSize(
                R.styleable.RadiosImageView_left_bottom_radius, 0);
        mRightBottomCornerRadius = a.getDimensionPixelSize(
                R.styleable.RadiosImageView_right_bottom_radius, 0);
        ...
        mBorderWidth = a.getDimensionPixelSize(
                R.styleable.RadiosImageView_radios_border_width, 0);
        if (mBorderWidth < 0) {
            throw new IllegalArgumentException("border width cannot be negative.");
        }

        mBorderColor = a
                .getColorStateList(R.styleable.RadiosImageView_radios_border_color);
        if (mBorderColor == null) {
            mBorderColor = ColorStateList.valueOf(DEFAULT_BORDER_COLOR);
        }

        isOval = a.getBoolean(R.styleable.RadiosImageView_radios_oval, false);
        a.recycle();
