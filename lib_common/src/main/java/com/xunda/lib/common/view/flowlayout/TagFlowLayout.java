package com.xunda.lib.common.view.flowlayout;
...
public class TagFlowLayout extends FlowLayout
    ...
    public void setAdapter(TagAdapter adapter,int index,boolean haveDefault) {
        mTagAdapter = adapter;
        mTagAdapter.setOnDataChangedListener(this);
        mSelectedView.clear();
        changeAdapter(index,haveDefault);
    }

    @SuppressWarnings("ResourceType")
    private void changeAdapter(int index,boolean haveDefault) {
        removeAllViews();
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            if(haveDefault){//需要设定默认值时才设定，否则不需要
                if(position==index){//刚进来时默认第一个选中
            ...
            tagViewContainer.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if(!finalTagViewContainer.isChecked()){//只有未被选中的才能点击
                        doSelect(finalTagViewContainer, position);
