 ...
 * 社群话题  转发资讯
 */
public class Group_MoreItem_Combination extends BaseItemProvider<OwnPublishBean, BaseViewHolder> {
    private int type;
    ...
    @Override
    public int layout() {
        return R.layout.group_moreitem_combination;
    }

    public Group_MoreItem_Combination(int type, String role) {
        this.type = type;
        this.role_self = role;
...
//            addRecycleData(mNineView, item,more_Tv);
            ...
            handleCommonData(item.getAvatar(), item.getNickname(), item.getContent(), item.getRole(), item.getAuthorId(), item.getStatusNum()
                    , group_Img, group_Name, tv_role, iv_delete, follow_Tv, send_Mes);
...
//            title_Tv.setVisibility(View.GONE);
    ...
    private void handleCommonData(String headImg, String headName, String content, String role, String publish_user_id, String follow_status
            , ImageView iv_headImg, TextView tv_headName, TextView tv_role, ImageView iv_delete, TextView tv_follow, TextView tv_send_mes) {
