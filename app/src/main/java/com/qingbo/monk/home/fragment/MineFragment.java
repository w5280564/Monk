...
//                        originalValue(userBean.getDescription(), "暂未填写", "个人说明：", brief_Tv);
                    ...
                    public void onComplete(String json_root, int code, String msg, String json_data) {
                        if (code == Constants.REQUEST_SUCCESS_CODE) {
                            ...
                            if (myCardBean != null) {
                                ...
                                String format = String.format("创建%1$s个社群", myCardBean.getData().getUser_group_count());
    ...
    private void showShareDialog() {
        if (userBean != null) {
            ...
            MShareDialog mShareDialog = new MShareDialog(requireActivity(), downURl, imgUrl, title, content, "分享");
            mShareDialog.show();
        }
    }
